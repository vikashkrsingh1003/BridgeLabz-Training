	...
	public ByteArrayInputStream exportNotes(List<Note> notes) {
		...
		try (Workbook workbook = new XSSFWorkbook(); ByteArrayOutputStream outputStream = new ByteArrayOutputStream()) {
			...
			for (Note note : notes) {
				...
				row.createCell(4).setCellValue(note.getState() != null ? note.getState().toString() : "");
			...
			// Auto-size columns (This automatically adjusts each column's width based on its content.)
			...
			throw new RuntimeException("Failed to export notes to Excel", e);
		}
	}
}